...
function renderResults() {
  ...
  if (rec && !pinnedBox && activeBox !== rec.box.id) { activeBox = rec.box.id; activeLoad = 0; selectedKey = null; }
  if (!results.some(r => r.box.id === activeBox)) activeBox = boxes[0].id;
  const chosen = results.find(r => r.box.id === activeBox);
  activeLoad = Math.min(activeLoad, Math.max(0, chosen.loads.length - 1));
  const load = chosen.loads[activeLoad];
  const envelope = load?.envelope || { length: 0, width: 0, height: 0 };
  ...
  const cards = results.map((r,i) => {
    ...
    return `<button type="button" data-select="${r.box.id}" class="compare-card ${r.box.id === activeBox ? 'active' : ''}" aria-pressed="${r.box.id === activeBox}"><span class="compare-top"><span class="number">0${i+1} / EQUIPMENT ${rec?.box.id===r.box.id?'<b>· BEST FIT</b>':''}</span><span class="status ${r.status}">${status}</span></span><strong>${r.box.name}</strong><span class="compare-spec">${fmt(r.box.length)} × ${fmt(r.box.width)} × ${fmt(r.box.height)} cm <b>·</b> ${fmt(r.box.payload/1000)}t</span><span class="mini" title="${esc(detail)}">${esc(detail)}</span><span class="usage-track"><span style="width:${floorPct}%"></span></span><span class="usage-label">첫 장비 바닥 점유 ${fmt(floorPct)}%</span></button>`;
  }).join('');
...
$('cargo-rows').addEventListener('change', e => { if (e.target.dataset.field) renderResults(); });$('cargo-rows').addEventListener('click', e => {
...
$('clearance').addEventListener('input', renderResults);$('add-cargo').addEventListener('click', () => { if (items.length>=20) return; items.push({ name:'',length:100,width:100,height:100,weight:100,qty:1,rotation:'floor',stackable:false }); renderItems(); renderResults(); $('cargo-rows').lastElementChild.querySelector('input').focus(); });$('example').addEventListener('click', () => { items=example(); boxes=PRESETS.map(p=>({...p})); $('clearance').value=0; activeBox='20DC'; activeLoad=0; pinnedBox=false; selectedKey=null; renderItems();renderBoxes();renderResults(); });$('result-content').addEventListener('click', e => {
...
renderItems(); renderBoxes(); renderResults();
